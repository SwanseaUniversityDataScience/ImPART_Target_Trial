...
CALL FNC.DROP_IF_EXISTS('SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR');

CREATE TABLE SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR AS
(SELECT coh.ALF_PE,
		coh.DIAG_DATE,
...
INSERT INTO SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR
SELECT coh.ALF_PE,
		coh.DIAG_DATE,
		wrrs.SPCM_COLLECTED_DT,
		wrrs.REPORT_SEQ,
...
DELETE FROM 
	(SELECT ROWNUMBER()	OVER(PARTITION BY ALF_PE ORDER BY SPCM_COLLECTED_DT) AS rn
			FROM SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR) AS mqo
			WHERE rn > 1;
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS');

CREATE TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS
AS (SELECT * FROM sailw1169v.VB_7day_TARGET_TRIAL_PRE2)
WITH NO DATA;

INSERT INTO sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS
SELECT * FROM sailw1169v.VB_7day_TARGET_TRIAL_PRE2;

--update target trial table with amr date

ALTER TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS
ADD COLUMN URINE_AMR_dt date
ADD COLUMN URINE_AMR integer;

MERGE INTO  sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS tt
	USING (SELECT DISTINCT alf_pe, spcm_collected_dt FROM SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR) AS amr
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS
...
DELETE FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS amr
...
(SELECT DISTINCT alf_pe,
		CASE WHEN sum(ltf) = 1
			...
			ELSE study_end
		END AS exit_dt
	FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS
...
INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS coh
...
MERGE INTO sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS coh
...
(SELECT cohp.*, pabxp.PABX_STR AS PABX_AFTER_RUTI1 FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS cohp
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS tt
SET death_after_outcome = 1
	WHERE tt.alf_pe IN 
	(SELECT DISTINCT coh.alf_pe FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS coh
INNER join
...
ALTER TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS activate NOT logged INITIALLY;			
					
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS t1
SET t1.next_uti_dt = 
(SELECT min(uti.EVENT_STR_DT)
	FROM SAILW1169V.VB_ALL_UTI_COMB AS uti
	INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS amr
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS amr
...
DELETE from sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS amr
WHERE amr.alf_pe IN (SELECT DISTINCT coh.alf_pe FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS  AS coh --working tab
INNER JOIN SAILW1169V.VB_ALTERNATING_PABX_COH3 AS pabx
ON coh.ALF_pe = pabx.ALF_PE
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_target_trial_urine_amr');

CREATE TABLE sailw1169v.VB_7day_target_trial_urine_amr
(ALF varchar(15),
RUTI_DIAGDATE date,
cal_time integer,
fup_obs integer,
...
INSERT INTO sailw1169v.VB_7day_target_trial_urine_amr
SELECT tt.alf_pe,
		tt.RUTI_DIAG_Dt,
		tt.cal_time,
		dur.inc_dur,
		...
		tt.next_uti
FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS AS tt
...
GRANT ALL ON sailw1169v.VB_7day_target_trial_urine_amr TO USER sanyaoll;
