...
--Urine AMR identified in WRRS

--identified AS ANY urine test of interest in identifying UTIs with a specimen collected date
--after the patient's recurrent UTI diagnosis.
--AMR established as resistant record for Nitrofurantoin, Trimethoprim or Cefalexin regardless 
--of UTI status of the test
...
CALL FNC.DROP_IF_EXISTS('SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_TRIM_RESIST');

CREATE TABLE SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_TRIM_RESIST AS
(SELECT coh.ALF_PE,
		coh.DIAG_DATE,
...
INSERT INTO SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_TRIM_RESIST
SELECT coh.ALF_PE,
		coh.DIAG_DATE,
		wrrs.SPCM_COLLECTED_DT,
		wrrs.REPORT_SEQ,
...
DELETE FROM 
	(SELECT ROWNUMBER()	OVER(PARTITION BY ALF_PE ORDER BY SPCM_COLLECTED_DT) AS rn
			FROM SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_TRIM_RESIST) AS mqo
			WHERE rn > 1;
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST');

CREATE TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST
AS (SELECT * FROM sailw1169v.VB_7day_TARGET_TRIAL_PRE2)
WITH NO DATA;

INSERT INTO sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST
SELECT * FROM sailw1169v.VB_7day_TARGET_TRIAL_PRE2;

--update target trial table with amr date

ALTER TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST
ADD COLUMN URINE_AMR_dt date
ADD COLUMN URINE_AMR integer;

MERGE INTO  sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS tt
	USING (SELECT DISTINCT alf_pe, spcm_collected_dt FROM SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_TRIM_RESIST) AS amr
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST
...
DELETE FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS amr
...
(SELECT DISTINCT alf_pe,
		CASE WHEN sum(ltf) = 1
			...
			ELSE study_end
		END AS exit_dt
	FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST
...
INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS coh
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_target_trial_closest_wrrs_trim_resist');

CREATE TABLE sailw1169v.VB_7day_target_trial_closest_wrrs_trim_resist
AS (SELECT alf_pe,
			spcm_collected_dt,
			uti_outcome,
			alf_pe AS UTI_IND
...
INSERT INTO sailw1169v.VB_7day_target_trial_closest_wrrs_trim_resist
WITH cte as
(SELECT coh.alf_pe, coh.RUTI_DIAG_DT, max(SPCM_COLLECTED_DT) AS closest_spec_dt FROM SAILW1169V.COHORT_WRRS_RESULTS_AGREED AS wrrs
INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS coh
ON wrrs.alf_pe = coh.ALF_PE
...
MERGE INTO sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS coh
	USING sailw1169v.VB_7day_target_trial_closest_wrrs_trim_resist AS wr
...
(SELECT cohp.*, pabxp.PABX_STR AS PABX_AFTER_RUTI1 FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS cohp
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS tt
SET death_after_outcome = 1
	WHERE tt.alf_pe IN 
	(SELECT DISTINCT coh.alf_pe FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS coh
INNER join
...
ALTER TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST activate NOT logged INITIALLY;			
					
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS t1
SET t1.next_uti_dt = 
(SELECT min(uti.EVENT_STR_DT)
	FROM SAILW1169V.VB_ALL_UTI_COMB AS uti
	INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS amr
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS amr
...
DELETE from sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS amr
WHERE amr.alf_pe IN (SELECT DISTINCT coh.alf_pe FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST  AS coh --working tab
INNER JOIN SAILW1169V.VB_ALTERNATING_PABX_COH3 AS pabx
ON coh.ALF_pe = pabx.ALF_PE
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_target_trial_urine_amr_trim_resist');

CREATE TABLE sailw1169v.VB_7day_target_trial_urine_amr_trim_resist
(ALF varchar(15),
RUTI_DIAGDATE date,
cal_time integer,
fup_obs integer,
...
INSERT INTO sailw1169v.VB_7day_target_trial_urine_amr_trim_resist
SELECT tt.alf_pe,
		tt.RUTI_DIAG_Dt,
		tt.cal_time,
		dur.inc_dur,
		...
		tt.next_uti
FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_TRIM_RESIST AS tt
...
GRANT ALL ON sailw1169v.VB_7day_target_trial_urine_amr_trim_resist TO USER sanyaoll;
...
CALL FNC.DROP_IF_EXISTS('SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_NITRO_RESIST');

CREATE TABLE SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_NITRO_RESIST AS
(SELECT coh.ALF_PE,
		coh.DIAG_DATE,
		wrrs.SPCM_COLLECTED_DT,
		wrrs.REPORT_SEQ,
...
INSERT INTO SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_NITRO_RESIST
SELECT coh.ALF_PE,
		coh.DIAG_DATE,
		wrrs.SPCM_COLLECTED_DT,
		wrrs.REPORT_SEQ,
...
DELETE FROM 
	(SELECT ROWNUMBER()	OVER(PARTITION BY ALF_PE ORDER BY SPCM_COLLECTED_DT) AS rn
			FROM SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_NITRO_RESIST) AS mqo
			WHERE rn > 1;
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST');

CREATE TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST
AS (SELECT * FROM sailw1169v.VB_7day_TARGET_TRIAL_PRE2)
WITH NO DATA;

INSERT INTO sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST
SELECT * FROM sailw1169v.VB_7day_TARGET_TRIAL_PRE2;

--update target trial table with amr date

ALTER TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST
ADD COLUMN URINE_AMR_dt date
ADD COLUMN URINE_AMR integer;

MERGE INTO  sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS tt
	USING (SELECT DISTINCT alf_pe, spcm_collected_dt FROM SAILW1169V.VB_TARGET_TRIAL_WRRS_AMR_NITRO_RESIST) AS amr
		ON tt.alf_pe = amr.alf_pe
			WHEN MATCHED THEN
				UPDATE
				SET tt.URINE_AMR_dt = amr.spcm_collected_dt
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST
SET URINE_AMR = CASE WHEN URINE_AMR_dt IS null
						THEN 0
				WHEN (URINE_AMR_dt > dod
					OR URINE_AMR_dt > GP_END_DT
...
DELETE FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS amr
    WHERE URINE_AMR_dt NOT BETWEEN cal_dt AND CAL_dt + 6 days
	AND cal_dt >= URINE_AMR_dt;
...
(SELECT DISTINCT alf_pe,
		CASE WHEN sum(ltf) = 1
			...
			ELSE study_end
		END AS exit_dt
	FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST
GROUP BY alf_pe, gp_end_dt, gp_end_dt, URINE_AMR_dt, dod, study_end, next_pabx_str, ruti_diag_dt
ORDER BY alf_pe) AS ex
ON tt.alf_pe = ex.alf_pe;
...
WITH cte as
(SELECT coh.alf_pe, coh.RUTI_DIAG_DT, max(SPCM_COLLECTED_DT) AS closest_spec_dt FROM SAILW1169V.COHORT_WRRS_RESULTS_AGREED AS wrrs
INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS coh
ON wrrs.alf_pe = coh.ALF_PE
WHERE wrrs.SPCM_COLLECTED_DT BETWEEN coh.RUTI_DIAG_DT - 1 YEAR AND coh.RUTI_DIAG_DT
GROUP BY coh.alf_pe, coh.RUTI_DIAG_DT),
cte2 as
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_target_trial_closest_wrrs_NITRO_RESIST');

CREATE TABLE sailw1169v.VB_7day_target_trial_closest_wrrs_NITRO_RESIST
AS (SELECT alf_pe,
			spcm_collected_dt,
			uti_outcome,
			alf_pe AS UTI_IND
...
INSERT INTO sailw1169v.VB_7day_target_trial_closest_wrrs_NITRO_RESIST
WITH cte as
(SELECT coh.alf_pe, coh.RUTI_DIAG_DT, max(SPCM_COLLECTED_DT) AS closest_spec_dt FROM SAILW1169V.COHORT_WRRS_RESULTS_AGREED AS wrrs
INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS coh
ON wrrs.alf_pe = coh.ALF_PE
...
MERGE INTO sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS coh
	USING sailw1169v.VB_7day_target_trial_closest_wrrs_NITRO_RESIST AS wr
		ON coh.ALF_PE = wr.ALF_PE
			WHEN MATCHED THEN
				UPDATE
				SET coh.MOST_RECENT_URINE = wr.UTI_IND
...
from
(SELECT mqo.ALF_PE, min(mqo.PABX_AFTER_RUTI1) AS PABX_AFTER_RUTI1 FROM
(SELECT cohp.*, pabxp.PABX_STR AS PABX_AFTER_RUTI1 FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS cohp
LEFT JOIN SAILW1169V.VB_ALTERNATING_PABX_COH3 AS pabxp
ON cohp.ALF_PE = pabxp.ALF_PE
AND MONTHS_BETWEEN(pabxp.PABX_STR,cohp.ruti_diag_dt) >= 0
AND MONTHS_BETWEEN(pabxp.PABX_STR,cohp.ruti_diag_dt) < 12
...
----------------------------------------------------------------------------------
--add column to identify whether death occurs between amr_outcome and study end date or gp_end.
--do not flag if lost to follow up due to gp_end prior to study end in case they have moved 
--out of the country and we would not have follow up data.

ALTER table sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST
ADD COLUMN death_after_outcome integer;

UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS tt
SET death_after_outcome = 1
	WHERE tt.alf_pe IN 
	(SELECT DISTINCT coh.alf_pe FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS coh
INNER join
(SELECT * FROM (SELECT alf_pe, min(COALESCE(gp_end_dt, STUDY_END)) AS min_end
FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST
GROUP BY alf_pe) AS mqo) AS jj
...
ALTER TABLE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST activate NOT logged INITIALLY;			
					
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS t1
SET t1.next_uti_dt = 
(SELECT min(uti.EVENT_STR_DT)
	FROM SAILW1169V.VB_ALL_UTI_COMB AS uti
	INNER JOIN sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS amr
		ON uti.ALF_PE = amr.ALF_PE
		AND uti.EVENT_STR_DT > amr.RUTI_DIAG_DT
WHERE t1.alf_pe = uti.alf_pe
GROUP BY uti.alf_pe);
...
UPDATE sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS amr
SET next_uti = case when amr.next_uti_dt
					BETWEEN cal_dt AND cal_dt + 6 DAYS THEN 1
						ELSE 0
				end;
...
DELETE from sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS amr
WHERE amr.alf_pe IN (SELECT DISTINCT coh.alf_pe FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST  AS coh --working tab
INNER JOIN SAILW1169V.VB_ALTERNATING_PABX_COH3 AS pabx
ON coh.ALF_pe = pabx.ALF_PE
AND coh.PABX_STR = pabx.PABX_STR
WHERE pabx.ALT_STR_DT < ruti_diag_dt
...
CALL fnc.drop_if_exists('sailw1169v.VB_7day_target_trial_urine_amr_NITRO_RESIST');

CREATE TABLE sailw1169v.VB_7day_target_trial_urine_amr_NITRO_RESIST
(ALF varchar(15),
RUTI_DIAGDATE date,
cal_time integer,
fup_obs integer,
...
INSERT INTO sailw1169v.VB_7day_target_trial_urine_amr_NITRO_RESIST
SELECT tt.alf_pe,
		tt.RUTI_DIAG_Dt,
		tt.cal_time,
		dur.inc_dur,
		...
		tt.death_after_outcome,
		tt.next_uti
FROM sailw1169v.VB_7day_TARGET_TRIAL_AMR_WRRS_NITRO_RESIST AS tt
	INNER JOIN sailw1169v.VB_7day_target_trial_pre AS pre
	ON tt.alf_pe = pre.alf_pe
	INNER JOIN SESSION.VB_Inc_duration AS dur
	ON tt.alf_pe = dur.alf_pe
...
GRANT ALL ON sailw1169v.VB_7day_target_trial_urine_amr_NITRO_RESIST TO USER sanyaoll;
